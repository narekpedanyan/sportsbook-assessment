...
const BetSlipStakeForm = memo(({ minStake, maxStake, currencySymbol }: BetSlipStakeFormProps) => {
  ...
  return (
    <div>
      <label className="mb-1 block text-sm font-medium">Stake ({currencySymbol})</label>
      <input
        type="number"
        step="0.01"
        min={minStake}
        ...
        {...register('stake')}
      />
      {errors.stake && <p className="text-destructive mt-1 text-xs">{errors.stake.message}</p>}
    </div>
  )
})
